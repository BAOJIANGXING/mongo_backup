...
class DatabaseBackup(object):
    def __init__(self, section, logger):
        ...
        self.mongo_dump_path = path.abspath(path.join(path.dirname(__file__), 'bin/mongodump'))
        ...
        try:
            if int(config.get('Proxy_Server', 'proxy_enabled')) == 1:
                ...
                if self.server.is_active:
                    self.logger('本地端口:{}已转发至远程端口{}:{}'.format(self.server.local_bind_port, self.proxy_host, self.proxy_port))
                else:
